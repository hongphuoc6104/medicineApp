...
def run_real_layout_ablation(
    ...
    # Special Focus: RX_001 & RX_019 & multi-capture prescriptions
    print("\n--- Special Prescription Focus ---")
    for target_pid in ["RX_001", "RX_019", "RX_016", "RX_023", "RX_002", "RX_003"]:
        if target_pid in per_prescription_stats:
            p_stat = per_prescription_stats[target_pid]
            print(f"\n▶ Prescription: {target_pid}")
            for s_key in strategy_keys:
                strat = strat_map[s_key]
                tp = p_stat[s_key]["tp"]
                fp = p_stat[s_key]["fp"]
                gt = p_stat[s_key]["gt_total"]
                rec = tp / gt if gt > 0 else 0.0
                prec = tp / (tp + fp) if (tp + fp) > 0 else 0.0
                f1 = (2 * prec * rec) / (prec + rec) if (prec + rec) > 0 else 0.0
                print(f"  - {strat:<20}: Recall = {rec*100:5.2f}% | Prec = {prec*100:5.2f}% | F1 = {f1*100:5.2f}% (TP={tp}, FP={fp}, GT={gt})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rx", type=str, default=None, help="Evaluate specific prescription ID (e.g. RX_001)")
