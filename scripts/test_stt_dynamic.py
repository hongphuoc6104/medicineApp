...
def group_by_stt_dynamic(blocks: list) -> list:
    ...
    # 3. Phân band
    boundaries = [(_y_center(anchors[i].bbox) + _y_center(anchors[i+1].bbox)) / 2.0 for i in range(len(anchors)-1)]
    ...
    for b in blocks:
        ...
        if yc < _y_center(anchors[0].bbox) - 20:
            headers.append(b); continue
        ok = False
        for i, bd in enumerate(boundaries):
            if yc <= bd:
                bands[i].append(b); ok = True; break
        if not ok:
            bands[-1].append(b)
    ...
    for band_idx, band in enumerate(bands):
        ...
        parts = [" ".join(b.text.strip() for b in cols[c] if b.text.strip()) for c in sorted(cols) if cols[c]]
        res = " | ".join(p for p in parts if p)
        
        all_pts = [pt for b in band for pt in b.bbox]
        xs = [pt[0] for pt in all_pts]
        ys = [pt[1] for pt in all_pts]
        m_bbox = [[min(xs), min(ys)], [max(xs), min(ys)], [max(xs), max(ys)], [min(xs), max(ys)]]
...
if __name__ == "__main__":
    file_path = "/home/hongphuoc/Desktop/medicineApp/data/output/phase_a/IMG_20260209_180423/step-3.2_ocr.json"
    ...
    print(f"Khoảng X biên của các cột (Tỉ lệ %):  {[round((x - min(all_x))/W, 2) for x in dynamic_bounds]}")
