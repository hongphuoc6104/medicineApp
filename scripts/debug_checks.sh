...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/debug_checks.sh [options]

Options:
  --quick            Run focused checks (default)
  --full             Run broader checks, including full Node suite
  --scan-smoke       Run Python app-path smoke script if available
  --image <path>     Run protected CLI pipeline on one image after checks (deprecated)
...
Examples:
  bash scripts/debug_checks.sh --quick
  bash scripts/debug_checks.sh --full --scan-smoke
EOF
}
...
section "Workspace debug checks: $MODE"
...
if [[ "$RUN_PYTHON" -eq 1 ]]; then
  ...
  if [[ "$RUN_SCAN_SMOKE" -eq 1 && -f "$ROOT_DIR/scripts/tests/test_api_alignment.py" ]]; then
    section "Python scan app-path smoke"
    run_in_dir "$ROOT_DIR" "$VENV_PY" scripts/tests/test_api_alignment.py
  fi
fi

if [[ "$RUN_NODE" -eq 1 ]]; then
  if [[ "$MODE" == "quick" ]]; then
    section "Node focused scan tests"
