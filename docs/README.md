# Docs — Tài Liệu

> Tài liệu tham khảo từ các giai đoạn phát triển khác nhau của dự án.

## Tài liệu chính

| File | Nội dung |
|------|----------|
| `master_plan.md` | Kế hoạch tổng thể dự án MedicineApp — kiến trúc, pipeline, phases, Flutter App |
| `project_status.md` | Trạng thái hiện tại: việc đã hoàn thành, đang chạy, chưa làm |
| `COLAB_SETUP.md` | Hướng dẫn setup training Zero-PIMA trên Google Colab |
| `OCR_METHOD_DECISION.md` | So sánh các phương pháp OCR đã thử (PaddleOCR, VietOCR, Hybrid) |
| `resume_on_new_account.md` | Hướng dẫn resume training khi đổi Colab account |

## Notebooks & Scripts Training

| File | Nội dung |
|------|----------|
| `train_zero_pima.py` | Script training Zero-PIMA (chạy local hoặc Colab) |
| `train_zero_pima.ipynb` | Notebook training Zero-PIMA (VAIPE dataset) |
| `resume_zero_pima_pro.ipynb` | Notebook resume training trên Colab Pro |

## Tham khảo

| File | Nội dung |
|------|----------|
| `Zero-Shot_Pill-Prescription_Matching_...pdf` | Paper gốc Zero-PIMA |
