...
\hypersetup{
  ...
  pdftitle={Báo cáo niên luận ngành Khoa học máy tính},
  pdfsubject={Hệ thống hỗ trợ quét đơn thuốc và lập kế hoạch dùng thuốc}
}
...
\chapter*{TÓM TẮT MỞ RỘNG}
\addcontentsline{toc}{chapter}{TÓM TẮT MỞ RỘNG}

Báo cáo nhấn mạnh định hướng triển khai hệ thống theo mô hình hỗ trợ người dùng, trong đó trí tuệ nhân tạo đóng vai trò xử lý đầu vào từ ảnh đơn thuốc và ứng dụng di động giữ vai trò kiểm soát cuối cùng trước khi lưu kế hoạch dùng thuốc. Hướng tiếp cận này giúp giảm thao tác nhập tay, đồng thời vẫn duy trì lớp kiểm tra thủ công để hạn chế rủi ro sai sót trong ngữ cảnh sử dụng y tế.

Về tổ chức hệ thống, giải pháp được chia thành ba lớp tách biệt gồm ứng dụng di động, máy chủ nghiệp vụ và dịch vụ trí tuệ nhân tạo. Chuỗi xử lý chính bao gồm phát hiện vùng đơn thuốc, tiền xử lý ảnh, nhận dạng ký tự quang học, trích xuất tên thuốc và đối sánh với cơ sở dữ liệu thuốc Việt Nam. Cách tổ chức theo lớp giúp dễ kiểm thử, dễ bảo trì và thuận lợi khi mở rộng chức năng trong các giai đoạn tiếp theo.

Kết quả thử nghiệm nội bộ cho thấy quy trình xử lý đạt độ ổn định tốt trên tập dữ liệu ảnh thực tế, đáp ứng đủ điều kiện để đưa sang bước lập lịch dùng thuốc. Bên cạnh đó, báo cáo cũng nêu rõ các giới hạn học thuật hiện tại như chưa có bộ chỉ số độ chính xác và độ bao phủ ở mức toàn quy trình và chưa tích hợp chức năng xác minh viên thuốc qua hình ảnh do yêu cầu an toàn sử dụng còn cao.

\noindent\textbf{Từ khóa:} đơn thuốc, nhận dạng ký tự quang học, trích xuất tên thuốc, ứng dụng di động, kế hoạch dùng thuốc, an toàn sử dụng.
...
\noindent
\begin{tabularx}{\textwidth}{C{2.6cm}Y}
\toprule
...
NER & Nhận dạng thực thể tên \\
OCR & Nhận dạng ký tự quang học \\
REST & Kiểu giao tiếp dữ liệu REST \\
ROI & Vùng quan tâm \\
YOLO & Thuật toán phát hiện đối tượng thời gian thực YOLO \\
\bottomrule
\end{tabularx}}
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{L{4.5cm}L{4cm}Y}
\toprule
...
\chapter[PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG]{PHÂN TÍCH VÀ\\[2pt]THIẾT KẾ HỆ THỐNG}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.98\textwidth]{assets/diagrams/use_case.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.92\textwidth,height=0.88\textheight,keepaspectratio]{assets/diagrams/scan_flow.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.96\linewidth,height=0.88\textheight,keepaspectratio]{assets/diagrams/erd_main.png}
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{L{8cm}Y}
\toprule
