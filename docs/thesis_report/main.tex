...
This report describes the design and development of a system for scanning paper prescriptions and generating medication schedules, using Optical Character Recognition (OCR) and artificial intelligence techniques. The problem arises from the everyday difficulty that patients—especially elderly users—face when reading prescriptions and keeping track of their daily medication intake. The main goal of the system is to convert prescription images into structured drug lists, allowing users to review and create medication plans without manual data entry.

The system is organized into three separate components. The mobile application handles the user interface, including camera capture, result display, and schedule setup. The Node.js server manages scan history, medication plans, and intake logs. The AI service runs a sequential processing pipeline covering prescription region detection, image preprocessing, OCR, drug-name entity extraction using a fine-tuned PhoBERT model, and fuzzy matching against a Vietnamese pharmaceutical database.

Internal evaluation on a set of 50 real prescription images showed a 100\% processing success rate with zero system errors, and an average of 6.8 drug names extracted per image. These results indicate that the recognition pipeline produces output suitable for the scheduling workflow. The report also discusses the limitations of the current evaluation, particularly the absence of precision and recall measurements at the full-pipeline level, and explains why two planned extensions—full-document OCR and visual pill verification—were not included in the main product due to safety and reliability concerns.
...
\begin{longtable}{|>{\centering\arraybackslash}p{2.5cm}|m{11.5cm}|}
\hline
\centering\textbf{Từ viết tắt} & \centering\arraybackslash\textbf{Diễn giải} \\
\hline
\endfirsthead
\hline
\centering\textbf{Từ viết tắt} & \centering\arraybackslash\textbf{Diễn giải} \\
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{4.5cm}|>{\raggedright\arraybackslash}p{4cm}|Y|}
\hline
\centering\textbf{Nhóm} & \centering\textbf{Công nghệ} & \centering\arraybackslash\textbf{Vai trò} \\
\hline
...
\hline
Dịch vụ trí tuệ nhân tạo & FastAPI \cite{fastapi} & Xử lý ảnh đơn thuốc và cung cấp API cho máy chủ chính. \\
\hline
...
\chapter[PHÂN TÍCH VÀ THIẾT KẾ HỆ THỐNG]{PHÂN TÍCH\\ VÀ THIẾT KẾ HỆ THỐNG}
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{1.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|Y|}
\hline
\centering\textbf{Mã} & \centering\textbf{Chức năng} & \centering\arraybackslash\textbf{Mô tả} \\
\hline
...
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{1.5cm}|>{\raggedright\arraybackslash}p{4.5cm}|Y|}
\hline
\centering\textbf{Bước} & \centering\textbf{Thành phần} & \centering\arraybackslash\textbf{Vai trò} \\
\hline
...
\chapter[CÀI ĐẶT HỆ THỐNG]{CÀI ĐẶT\\ HỆ THỐNG}
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{5.5cm}|Y|}
\hline
\centering\textbf{Thành phần} & \centering\arraybackslash\textbf{Mô tả} \\
\hline
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.5cm}|>{\centering\arraybackslash}p{4.5cm}|Y|}
\hline
\centering\textbf{Nhóm} & \centering\textbf{Ví dụ đường dẫn} & \centering\arraybackslash\textbf{Vai trò} \\
\hline
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{8cm}|>{\raggedleft\arraybackslash}X|}
\hline
\centering\textbf{Chỉ số} & \centering\arraybackslash\textbf{Giá trị ghi nhận} \\
\hline
...
Một giới hạn quan trọng cần làm rõ là đánh giá hiện tại chưa bao gồm các chỉ số độ chính xác và độ phủ (precision và recall) ở mức toàn bộ quy trình nhận diện. Báo cáo chỉ ghi nhận tỉ lệ xử lý thành công, tổng số thuốc trích xuất được và thời gian xử lý. Việc tính precision và recall đòi hỏi phải có tập dữ liệu kiểm thử có nhãn thủ công với danh sách thuốc chuẩn cho từng ảnh, điều mà đề tài chưa thực hiện được trong phạm vi thời gian hiện tại. Do đó, không thể kết luận định lượng về tỉ lệ nhận diện đúng hoặc tỉ lệ bỏ sót tên thuốc ở mức toàn pipeline. Đây là hướng cần bổ sung trong các đợt đánh giá tiếp theo nếu muốn nâng cao tiêu chuẩn học thuật của hệ thống.
...
K. Kesselheim và cộng sự, \textit{Patient views about generic medication appearance changes}. \textit{Annals of Internal Medicine}, 2019. [Trực tuyến]. Có tại: \url{https://pmc.ncbi.nlm.nih.gov/articles/PMC6420535/}. Truy cập: Tháng 4 năm 2026.
