...
Về kiến trúc, hệ thống được tổ chức thành ba thành phần tách biệt: ứng dụng di động phụ trách giao diện người dùng; máy chủ Node.js quản lý dữ liệu và nghiệp vụ; dịch vụ trí tuệ nhân tạo thực hiện chuỗi xử lý gồm phát hiện vùng đơn thuốc, tiền xử lý ảnh, OCR, gom khối OCR theo STT, trích xuất tên thuốc và đối sánh cơ sở dữ liệu thuốc Việt Nam.

Một benchmark nội bộ tiêu biểu ở giai đoạn hoàn thiện luồng Phase A ghi nhận pipeline xử lý trọn 50/50 ảnh thử nghiệm, nhận diện tổng cộng 338 mục thuốc, trung bình khoảng 6,8 thuốc trên mỗi ảnh. Các con số này được sử dụng như snapshot kỹ thuật của luồng chính tại thời điểm benchmark, không phải cam kết cho mọi bộ dữ liệu thực tế. Báo cáo cũng làm rõ các giới hạn còn lại, bao gồm việc chưa mở rộng OCR sang toàn bộ nội dung đơn và việc các thành phần xác minh viên thuốc trong repo mới dừng ở mức thử nghiệm, chưa được xem là luồng chính đã được kiểm chứng của niên luận.
...
This report presents the design and implementation of a system that supports prescription scanning and medication plan creation by combining optical character recognition and artificial intelligence techniques. The system is organized into three components: a mobile application for user interaction, a Node.js server for business logic and data management, and an AI service for prescription-region detection, image preprocessing, OCR, STT-based block grouping, drug-name extraction, and drug-database matching.

An internal benchmark snapshot recorded during the stabilization of the Phase A workflow shows that the pipeline completed 50/50 scans without runtime interruption, with an average of 6.8 recognized drugs per image. These values are reported as a technical snapshot of the validated core flow, not as a universal guarantee for all real-world datasets. The report also discusses current limitations, including the absence of manually labeled end-to-end accuracy metrics and the fact that pill-image verification artifacts in the repository are still experimental rather than part of the validated main workflow.
...
Một hướng khác cũng đã được khảo sát là chức năng xác minh viên thuốc qua hình ảnh. Repo hiện đã có một số route và màn hình thử nghiệm cho hướng này, nhưng chức năng đó chưa được xem là một phần của luồng chính đã được kiểm chứng trong niên luận vì vẫn còn rủi ro an toàn nếu hệ thống nhận nhầm các viên thuốc có ngoại hình gần giống nhau. Nội dung này được phân tích như một giới hạn và hướng phát triển ở các chương sau.
...
Về kỹ thuật, quy trình xử lý quét đơn thuốc được tổ chức theo chuỗi: phát hiện vùng đơn thuốc, tiền xử lý ảnh, kiểm tra chất lượng, OCR, gom khối OCR theo STT, trích xuất tên thuốc và chuẩn hóa tên thuốc. Ở phía ứng dụng, kết quả nhận diện luôn đi qua bước rà soát lại trước khi lập lịch nhằm giảm rủi ro sai sót trong ngữ cảnh y tế.
...
7 & Lịch sử và theo dõi & Người dùng xem lịch sử kế hoạch, nhật ký dùng thuốc và kế hoạch đang hoạt động. \\
\bottomrule
\end{tabularx}}
\end{table}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.96\textwidth]{assets/diagrams/use_case_a4_v3.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.94\textwidth]{assets/diagrams/architecture_a4_v3.png}
...
Trong luận văn này, luồng quét đơn thuốc được mô tả theo nhánh một ảnh một lần quét, vì đây là nhánh đang được dùng làm luồng trung tâm để đánh giá từ đầu vào ảnh đến bước rà soát danh sách thuốc. Các endpoint kiểu scan session thích nghi vẫn tồn tại ở backend như hướng mở rộng, nhưng không phải nhánh chính được minh họa ở sơ đồ sau.

Luồng quét đơn thuốc được thiết kế theo thứ tự sau:

\begin{enumerate}[label=\arabic*.]
    \item Người dùng mở màn hình quét và chụp ảnh đơn thuốc bằng camera.
...
Sự phối hợp giữa các tầng khi có luồng quét đơn thuốc diễn ra thông qua chuỗi trao đổi dữ liệu liên tục được phản ánh rõ ở Sơ đồ tuần tự minh họa trong Hình~\ref{fig:sequence-scan}. Ở phiên bản cập nhật này, sơ đồ đã được rút gọn theo đúng luồng core hiện tại để dễ đọc trên một trang A4 landscape và tránh trộn lẫn với các endpoint phụ chưa phải trọng tâm của luận văn.

\begin{landscape}
\begin{figure}[p]
\centering
\includegraphics[width=0.98\linewidth,height=0.82\textheight,keepaspectratio]{assets/diagrams/sequence_scan_a4_v3.png}
\caption{Sơ đồ tuần tự của luồng quét đơn thuốc trong nhánh đánh giá trung tâm}
\label{fig:sequence-scan}
\end{figure}
\end{landscape}

Giai đoạn phức tạp nhất là khâu phân tích ảnh tại dịch vụ AI. Hình~\ref{fig:scan-flow} đi sâu vào chuỗi xử lý bên trong FastAPI, làm rõ ba quyết định kỹ thuật quan trọng của phiên bản hiện tại: chặn sớm các ảnh bị từ chối ở quality gate, dùng Table ROI theo hướng tùy chọn có fallback, và gom các khối OCR theo STT trước khi đưa sang bước trích xuất tên thuốc. Sơ đồ cũng được vẽ lại theo bố cục ngang để khi đặt trên một trang A4 landscape vẫn giữ được cỡ chữ dễ đọc.

\begin{landscape}
\begin{figure}[p]
\centering
\includegraphics[width=0.98\linewidth,height=0.82\textheight,keepaspectratio]{assets/diagrams/scan_flow_a4_v3.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.98\linewidth,height=0.82\textheight,keepaspectratio]{assets/diagrams/erd_main_a4_v3.png}
...
Đề tài đã khảo sát hướng xác minh viên thuốc bằng ảnh đầu vào của người dùng, kết hợp đối sánh theo tên thuốc, màu sắc, hình dạng và dạng bào chế. Tuy nhiên, hướng này mới dừng ở mức thử nghiệm trong repo và chưa được đưa vào phạm vi đánh giá trung tâm vì độ an toàn chưa đủ cao. Phần phân tích chi tiết được trình bày ở Chương 5.
...
Mô-đun quét đơn thuốc được triển khai trong dịch vụ trí tuệ nhân tạo viết bằng Python. Thành phần điều phối có nhiệm vụ nối các bước xử lý thành một chuỗi thống nhất. Ở mức tách lớp, các bước detect/crop, tiền xử lý, quality gate, OCR, gom khối OCR theo STT, trích xuất tên thuốc và tra cứu thuốc được đặt ở các thư mục riêng để dễ gỡ lỗi và kiểm thử.

Trong quá trình triển khai, bước phát hiện đơn thuốc bằng YOLO được dùng để tách phần tờ đơn ra khỏi nền ảnh theo đường bao lồi. Sau đó ảnh được deskew theo Modulo 90 và hiệu chỉnh hướng bằng mô hình AI để bộ OCR hoạt động ổn định hơn. Sau quality gate, hệ thống có thể dò Table ROI theo hướng tùy chọn trước khi đưa qua OCR. Kết quả nhận dạng tiếp tục được gom theo STT, rồi mới đi qua PhoBERT để lọc ra các đoạn có khả năng là tên thuốc trước khi đối sánh với cơ sở dữ liệu thuốc Việt Nam.
...
Trong luồng quét đơn thuốc, máy chủ nhận ảnh từ ứng dụng di động, chuyển tiếp sang FastAPI dưới dạng tệp tải lên, sau đó chuẩn hóa kết quả nhận diện về một cấu trúc ổn định hơn cho ứng dụng di động. Máy chủ cũng lưu lịch sử quét, trạng thái chất lượng ảnh và các thông tin tổng hợp khác phục vụ cho API history cũng như nhu cầu theo dõi hệ thống ở backend.

Các nhóm API ở thời điểm hiện tại có thể chia thành hai lớp: lớp phục vụ trực tiếp cho luồng scan -> review -> schedule -> track của luận văn, và lớp phụ trợ hoặc thử nghiệm đã tồn tại trong repo nhưng không phải trọng tâm đánh giá. Bảng~\ref{tab:api-groups} tóm tắt các nhóm này.
...
{\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{C{1.0cm}L{2.9cm}L{4.1cm}Y}
\toprule
\textbf{STT} & \textbf{Nhóm} & \textbf{Ví dụ đường dẫn} & \textbf{Vai trò} \\
\midrule
1 & Health & \path{/api/health} & Kiểm tra trạng thái sẵn sàng của backend. \\
2 & Xác thực & \texttt{/api/auth/...} & Đăng ký, đăng nhập, refresh token và quản lý người dùng. \\
3 & Quét đơn core & \path{/api/scan}, \path{/api/scan/history/...} & Nhận ảnh đơn thuốc, lưu lịch sử quét và trả kết quả cho màn hình rà soát. \\
4 & Scan session mở rộng & \path{/api/scan/session/...} & Hỗ trợ nhánh quét thích nghi nhiều ảnh; tồn tại trong repo nhưng không phải luồng minh họa chính của luận văn. \\
5 & Kế hoạch và nhật ký & \path{/api/plans/...} & Quản lý kế hoạch, liều dùng trong ngày, các endpoint today summary và trạng thái \texttt{taken/missed/skipped}. \\
6 & Tra cứu thuốc & \path{/api/drugs/...}, \path{/api/drug-interactions/...} & Tìm kiếm thuốc, tra cứu hoạt chất và kiểm tra tương tác như các chức năng phụ trợ. \\
7 & Pill verification & \path{/api/pill-verifications/...} & Thành phần thử nghiệm cho hướng xác minh viên thuốc; chưa thuộc luồng đánh giá trung tâm. \\
8 & Pill reference & \path{/api/pill-references/...} & Quản lý ảnh tham chiếu viên thuốc cho hướng thử nghiệm tương ứng. \\
\bottomrule
\end{tabularx}}
\end{table}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.46\textwidth]{assets/app/scan_camera.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.46\textwidth]{assets/app/scan_review.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.46\textwidth]{assets/app/set_schedule.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.46\textwidth]{assets/app/home_today.png}
...
Khi đưa dữ liệu vào báo cáo, đề tài ưu tiên trình bày kết quả ở dạng tổng hợp thay vì công bố trực tiếp toàn bộ nội dung đơn thuốc. Với các hình minh họa đã sử dụng trong luận văn, các vùng có khả năng làm lộ thông tin cá nhân như họ tên, địa chỉ hoặc mã số khám bệnh tiếp tục được che hoặc lược bỏ.
...
Một hướng mở rộng khác của đề tài là xây dựng chức năng xác minh viên thuốc qua hình ảnh. Ở trạng thái hiện tại của repo, hướng này đã có route và màn hình thử nghiệm riêng, nhưng chúng chưa được xem là một phần của luồng trung tâm đã được kiểm chứng trong luận văn. Lý do cốt lõi là bài toán này có rủi ro an toàn cao nếu chưa đạt độ tin cậy đủ mạnh.
...
Vì vậy, dù hướng xác minh viên thuốc qua hình ảnh là một hướng nghiên cứu có giá trị, luận văn này chỉ xem đó là hướng mở rộng đang ở trạng thái thử nghiệm, chưa đủ an toàn để được gộp vào luồng chính đã được đánh giá. Các nguyên nhân chính gồm:
...
Một benchmark snapshot nội bộ trên 50 ảnh đơn thuốc thực tế cho thấy hệ thống đạt tỉ lệ xử lý thành công 100\%, không phát sinh lỗi dừng xử lý, và trung bình nhận diện 6,8 thuốc trên mỗi ảnh. Luồng ứng dụng từ quét đơn đến lưu kế hoạch và theo dõi uống thuốc đã hoạt động hoàn chỉnh. Ở mặt kỹ thuật phần mềm, backend và pipeline Python đều được bao phủ bởi các bộ kiểm thử tự động đang tiếp tục được mở rộng theo tiến độ phát triển. Những kết quả này cho thấy hệ thống đáp ứng được các mục tiêu ban đầu ở mức thực nghiệm nội bộ.

\section{Những nội dung chưa nằm trong phạm vi đánh giá trung tâm}

Trong quá trình thực hiện, đề tài có xem xét một số hướng mở rộng như trích xuất thêm thông tin bệnh nhân, triệu chứng, chẩn đoán và xác minh viên thuốc qua hình ảnh. Một phần trong số đó hiện đã có artifact ở mức route, màn hình hoặc bảng dữ liệu trong repo. Tuy nhiên, thay vì đưa tất cả vào phần đánh giá trung tâm, luận văn chọn cách thu hẹp phạm vi để bảo toàn tính ổn định và tính thực dụng của luồng chính.
