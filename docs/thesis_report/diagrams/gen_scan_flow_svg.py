...
def diamond(svg, cx, cy, w, h, lines, fill="#fff8e1", stroke="#b7791f", size=13):
    ...
    ET.SubElement(
        svg, "polygon", points=pts, fill=fill, stroke=stroke, **{"stroke-width": "1.7"}
    )
    n = len(lines)
    for i, ln in enumerate(lines):
        dy = (i - (n - 1) / 2) * 16
...
def arrow(svg, x1, y1, x2, y2, label="", label_side="right", size=12):
    ...
    if label:
        ...
        lbl(svg, mx + off_x, my + off_y, label, size=size, italic=True, color="#555")
...
step_box(
    ...
    BW,
    BH,
    ["Bước 1: Phát hiện và cắt vùng đơn thuốc", "(YOLO Detect — Convex Hull Crop)"],
    "#edf7f0",
    "#2d7a46",
    size=13,
)
...
step_box(
    ...
    BW,
    BH,
    ["Bước 2: Tiền xử lý ảnh", "(Chỉnh nghiêng Modulo 90 + Căn hướng AI)"],
    "#edf7f0",
    "#2d7a46",
    size=13,
)
...
step_box(
    ...
    BW,
    BH,
    ["Bước 3: Nhận dạng ký tự quang học", "(PaddleOCR + VietOCR)"],
    "#edf7f0",
    "#2d7a46",
    size=13,
)
...
step_box(
    ...
    BW,
    BH,
    ["Bước 4: Trích xuất tên thuốc", "(PhoBERT NER)"],
    "#edf7f0",
    "#2d7a46",
    size=13,
)
...
step_box(
    ...
    BW,
    BH,
    ["Bước 5: Tra cứu và chuẩn hóa tên thuốc", "(Cơ sở dữ liệu 9.284 thuốc Việt Nam)"],
    "#edf7f0",
    "#2d7a46",
    size=13,
)
...
arrow(
    svg, COL_L, y_qg + DH // 2, COL_L, y_b3 - BH // 2, "Đạt yêu cầu", label_side="right"
)
