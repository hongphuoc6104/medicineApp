...
for pid, cx, lines, fill, stroke, tc in participants:
    ...
    for i, ln in enumerate(lines):
        dy = (i - (n - 1) / 2) * 18
        lbl(svg, cx, PART_TOP + PART_H / 2 + dy + 1, ln, size=14, bold=True, color=tc)
    # lifeline
    ET.SubElement(
        svg,
        "line",
...
def draw_badge(svg, x, y, num):
    ...
    r = 12
    ET.SubElement(svg, "circle", cx=str(x), cy=str(y), r=str(r), fill="#1a3a6e")
...
def draw_message(svg, from_id, to_id, label, is_return, step_num):
    ...
    if from_id == to_id:
        ...
        lbl(svg, lx + 8, top + 16, label, size=12, color="#1a1a2e", anchor="start")
        ...
        ET.SubElement(
            ...
            y2=str(y),
            stroke="#37474f",
            **{"stroke-width": "1.5", "stroke-dasharray": dash, "marker-end": marker},
        )
        mid_x = (x1 + x2) / 2
