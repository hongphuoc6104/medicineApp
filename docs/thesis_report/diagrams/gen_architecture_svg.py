...
def layer_rect(svg, x, y, w, h, fill, stroke, title, title_color, rx=8):
    ET.SubElement(svg, "rect", x=str(x), y=str(y), width=str(w), height=str(h),
                  ...
                  **{"stroke-width": "1.8"})
    lbl(svg, x + 14, y + 21, title, size=12, bold=True,
...
for i, its in enumerate(items1):
    ...
    item_box(svg, bx, L1_Y + 38 + bh // 2, bw, bh,
...
for i, its in enumerate(items2):
    bx = PAD_X + 24 + i * (bw + gap_item) + bw // 2
    item_box(svg, bx, L2_Y + 38 + bh // 2, bw, bh,
...
for i, its in enumerate(items3):
    bx = PAD_X + 24 + i * (bw + gap_item) + bw // 2
    item_box(svg, bx, L3_Y + 40 + bh // 2, bw, bh + 10,
...
cylinder(svg, DB1_CX, DB_Y, 58, CYL_RH, CYL_BH,
         "#eef2ff", "#2457a5",
         ["PostgreSQL", "(Dữ liệu hệ thống)"], size=10)
...
lbl(svg, DB2_CX + 52, (L3_BOT + DB2_TOP) // 2,
