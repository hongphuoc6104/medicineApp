...
def test_crop_by_bbox_with_detection():
    ...
    assert crop.shape == (60, 50, 3), (
        "Kích thước ảnh crop bằng Bounding Box đang bị sai logic"
    )
...
def test_crop_by_mask_with_detection():
    ...
    fake_masks = MockMasks(
        xy_points=[[30, 40], [60, 40], [60, 70], [30, 70]],
        mask_data=fake_mask_data,
    )
    ...
    assert crop.shape == (71, 71, 3), "Kích thước ảnh crop theo Mask + Padding bị sai!"
...
def test_crop_by_mask_padding_clamp():
    ...
    # cv2.boundingRect() tính width/height inclusive, nên bbox 5..50 cho ra 46 px.
    # Shape cuối cùng là (46 + 20, 46 + 20, 3) = (71, 71, 3).
    assert crop.shape == (71, 71, 3), "Clamping logic bị bỏ qua, padding vượt biên ảnh!"
