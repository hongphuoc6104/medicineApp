...
4. `docs/UONG_THUOC_FULL_TEST_PLAN.md`
...
- mục tiêu sản phẩm
- phạm vi đã khóa
- trục phát triển chính
- thứ tự ưu tiên giữa các workstream
...
- các workstream active
- phạm vi điển hình của từng workstream
- tiêu chí xong và luật dừng

### Bước 4 — Chỉ giao đúng một workstream nhỏ hoặc lát cắt nhỏ

Execution model chỉ được giao:

- một mục tiêu rõ ràng
- một lát cắt nhỏ nằm trong đúng workstream active
...
Nếu execution spec cũ hoặc plan cũ đã hết vai trò active, test ổn và không còn được dùng nữa:

- chuyển file đó vào `archive/plan_bin/`

### 5.2 Khi toàn bộ general plan xong

Chỉ khi toàn bộ các workstream thuộc general plan đã hoàn thành ổn định thì mới:
...
```text
Đọc AGENTS.md, APP_ACTIVE_GENERAL_PLAN.md, APP_ACTIVE_DETAILED_PLAN.md và docs/phase_a_debug_runbook.md trước.

Chỉ làm đúng workstream hoặc lát cắt nhỏ đang được giao.
...
- chia việc đúng model và đúng workstream/lát cắt nhỏ
